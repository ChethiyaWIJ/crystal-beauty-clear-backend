 ...
 Order.find()
        ...
        .limit(1).then(async (lastBills)=>{
            if (lastBills.length == 0) {
                ...
                const lastOrderId = lastBill.orderId;//"ORD00001"
                const lastOrderNumber = lastOrderId.replace("ORD","");//"0001"
                const lastOrderNumberInt = parseInt(lastOrderNumber);//01   
                const newOrderNumberInt = lastOrderNumberInt + 1;//02
...
export function getOrders(req,res) {
    ...
    if(req.user.role == "admin") {
        ...
        Order.find({
            email : req.user.email//orders sent by the user with the email
        }).then(
            (orders)=>{
                res.json(orders)
            }
